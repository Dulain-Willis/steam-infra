...
trap 'kill "$TUNNEL_PID" 2>/dev/null || true; pkill -f "localPortNumber.*15432" 2>/dev/null || true; rm -f "$TUNNEL_LOG"' EXIT
...
PGPASSWORD="$DB_PASSWORD" psql -h 127.0.0.1 -p 15432 -U steam_proj_admin -d steam \
