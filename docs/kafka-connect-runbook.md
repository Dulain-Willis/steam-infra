...
`steam_proj_admin` has `rds_replication` + `SELECT` on all 16 tables. Fails
...
Consumes the 16 `steam.public.*` Debezium topics directly and writes to
...
Topics created for all 16 tables (`topic.prefix: steam` +
...
Should list 16 topics, `steam.public.<table>` for each table in
`db/schema.sql`.
